...
const UsuarioInicial: AuthState = {
  isAutenticated: false,
  userName: '',
  userRol: ''
}

//Creamos los reducers dentro de la función createSlice y los asignamos a la variable sliceEstado:
//. Primero le damos un nombre
//. Segundo establecemos el estado inicial
//. Tercero creamos los reducers: login y logout
const sliceEstado = createSlice({
  name: 'authentication',
  ...
  reducers: {
    ...
    login: (state, action) => {
      const infoUsuarios = action.payload //Obtenemos el nombre y rol de usuario y lo asignamos a la variable infoUsuarios
      state.isAutenticated = true //Establecemos a true isAutenticated en la store
      state.userName = infoUsuarios.name //Es lo mismo que: action.payload.name --> Establecemos el nombre de usuario en la store
      state.userRol = infoUsuarios.rol //Es lo mismo que: action.payload.rol --> Establecemos el rol de usuario en la store
    },
...
//Exportamos las acciones del reducer en la variable Acciones
export const Acciones = sliceEstado.actions

//Exportamos el reducer en sí
export default sliceEstado.reducer
